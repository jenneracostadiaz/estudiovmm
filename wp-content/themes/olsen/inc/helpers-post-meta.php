<?php
add_action( 'after_setup_theme', 'ci_theme_setup_helpers_post_meta' );
function ci_theme_setup_helpers_post_meta() {
	add_image_size( 'ci_featgal_small_thumb', 100, 100, true );

	add_action( 'wp_ajax_ci_theme_featgal_AJAXPreview', 'ci_theme_featgal_AJAXPreview' );
}

add_action( 'admin_enqueue_scripts', 'ci_theme_admin_register_post_meta_scripts' );
function ci_theme_admin_register_post_meta_scripts( $hook ) {
	$theme = wp_get_theme();

	wp_register_style( 'ci-theme-post-meta', get_template_directory_uri() . '/inc/css/post-meta.css', array(), $theme->get( 'Version' ) );
	wp_register_script( 'ci-theme-post-meta', get_template_directory_uri() . '/inc/js/post-meta.js', array(
		'media-editor',
		'jquery',
		'jquery-ui-sortable'
	), $theme->get( 'Version' ) );
	...
	$settings = array(
		...
		'tSelectFile'         => __( 'Select file', 'ci-theme' ),
		'tSelectFiles'        => __( 'Select files', 'ci-theme' ),
		'tUseThisFile'        => __( 'Use this file', 'ci-theme' ),
		'tUseTheseFiles'      => __( 'Use these files', 'ci-theme' ),
		'tUpdateGallery'      => __( 'Update gallery', 'ci-theme' ),
		'tLoading'            => __( 'Loading...', 'ci-theme' ),
		'tPreviewUnavailable' => __( 'Gallery preview not available.', 'ci-theme' ),
		'tRemoveImage'        => __( 'Remove image', 'ci-theme' ),
		'tRemoveFromGallery'  => __( 'Remove from gallery', 'ci-theme' ),
	);
	wp_localize_script( 'ci-theme-post-meta', 'ci_theme_PostMeta', $settings );
}
...
function ci_theme_prepare_metabox( $post_type ) {
	wp_nonce_field( basename( __FILE__ ), $post_type . '_nonce' );
}

function ci_theme_can_save_meta( $post_type ) {
	global $post;
...
function ci_theme_metabox_gallery( $gid = 1 ) {
	global $post;
	$post_id = $post->ID;

	ci_theme_featgal_print_meta_html( $post_id, $gid );
}

function ci_theme_metabox_gallery_save( $POST, $gid = 1 ) {
	global $post;
	$post_id = $post->ID;

	ci_theme_featgal_update_meta( $post_id, $POST, $gid );
}

function ci_theme_metabox_input( $fieldname, $label, $params = array() ) {
	global $post;
...
function ci_theme_metabox_textarea( $fieldname, $label, $params = array() ) {
	global $post;
...
function ci_theme_metabox_dropdown( $fieldname, $options, $label, $params = array() ) {
	global $post;
...
function ci_theme_metabox_radio( $fieldname, $optionname, $optionval, $label, $params = array() ) {
	global $post;
...
function ci_theme_metabox_checkbox( $fieldname, $value, $label, $params = array() ) {
	global $post;
...
function ci_theme_metabox_open_tab( $title ) {
	?>
...
function ci_theme_metabox_close_tab() {
	?>
		</div>
	</div>
	<?php
...
function ci_theme_metabox_open_collapsible( $title ) {
	?>
	<div class="postbox" style="margin-top:20px">
		<div class="handlediv" title="<?php esc_attr_e( 'Click to toggle', 'ci-theme' ); ?>"><br></div>
		<h3 class="hndle"><?php echo esc_html( $title ); ?></h3>
		<div class="inside">
	<?php
}

function ci_theme_metabox_close_collapsible() {
	?>
		</div>
	</div>
	<?php
...
function ci_theme_metabox_guide( $strings, $params = array() ) {
	$defaults = array(
...
function ci_theme_bind_metabox_to_page_template( $metabox_id, $template_file, $js_var ) {
...
ENDJS;
	...
	ci_theme_add_inline_js( $js, sanitize_key( 'metabox_template_' . $metabox_id . '_' . $template_file ) );
}

function ci_theme_bind_metabox_to_post_format( $metabox_id, $post_format, $js_var ) {
...
ENDJS;
	...
	ci_theme_add_inline_js( $js, sanitize_key( 'metabox_format_' . $metabox_id . '_' . $post_format ) );
}
 ...
 * @return void
 */
function ci_theme_featgal_print_meta_html( $post_id = false, $gid = 1 ) {
	if ( $post_id == false ) {
		global $post;
		$post_id = $post->ID;
	}
	...
	<div class="ci-media-manager-gallery">
		<input type="button" class="ci-upload-to-gallery button" value="<?php _e( 'Add Images', 'ci-theme' ); ?>"/>
		...
		<p><label class="ci-upload-to-gallery-random"><input type="checkbox" name="ci_featured_gallery_rand_<?php echo esc_attr( $gid ); ?>" value="rand" <?php checked( $rand, 'rand' ); ?> /> <?php _e( 'Randomize order', 'ci-theme' ); ?></label></p>
		<div class="ci-upload-to-gallery-preview group">
			<?php
				$images = ci_theme_featgal_get_images( $ids );
				if ( $images !== false and is_array( $images ) ) {
					foreach ( $images as $image ) {
						?>
						<div class="thumb">
							<img src="<?php echo esc_url( $image['url'] ); ?>" data-img-id="<?php echo esc_attr( $image['id'] ); ?>">
							<a href="#" class="close media-modal-icon" title="<?php echo esc_attr( __( 'Remove from gallery', 'ci-theme' ) ); ?>"></a>
						</div>
						<?php
					}
				}
			?>
			<p class="ci-upload-to-gallery-preview-text"><?php _e( 'Your gallery images will appear here', 'ci-theme' ); ?></p>
		</div>
	</div>
	<?php
}
 ...
 */
function ci_theme_featgal_update_meta( $post_id, $POST, $gid = 1 ) {
	if ( absint( $post_id ) < 1 ) {
		return false;
	}
...
function ci_theme_featgal_get_ids( $post_id = false, $gid = 1 ) {
	if ( $post_id == false ) {
		global $post;
		$post_id = $post->ID;
	} else {
...
function ci_theme_featgal_get_attachments( $post_id = false, $gid = 1, $extra_args = array() ) {
	if ( $post_id == false ) {
		global $post;
		$post_id = $post->ID;
	} else {
 ...
 * Hooked to wp_ajax_ci_theme_featgal_AJAXPreview for AJAX updating of the galleries' previews.
 */
function ci_theme_featgal_AJAXPreview() {
	$ids  = $_POST['ids'];
	$urls = ci_theme_featgal_get_images( $ids );
	if ( $urls === false ) {
		echo 'FAIL';
 ...
 * @return array|bool
 */
function ci_theme_featgal_get_images( $csv = false ) {
	$ids = explode(',', $csv);
	$ids = array_filter($ids);
	...
	if ( count( $ids ) > 0 ) {
		...
		foreach ( $ids as $id ) {
			$thumb_file = ci_theme_get_image_src( $id, 'ci_featgal_small_thumb' );
			...
			if ( ci_theme_substr_right( $file, strlen( $size ) ) == $size ) {
				$file = $thumb_file;
			} else {
				$file = ci_theme_get_image_src( $id, 'thumbnail' );
			}
