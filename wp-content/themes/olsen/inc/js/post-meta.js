jQuery( document ).ready( function( $ ) {
	...
	$( 'body' ).on( 'click', '.ci-media-button', function( e ) {
		...
		var ciMediaUpload = wp.media( {
			...
			title: bMulti == true ? ci_theme_PostMeta.tSelectFiles : ci_theme_PostMeta.tSelectFile,
			button: {
				text: bMulti == true ? ci_theme_PostMeta.tUseTheseFiles : ci_theme_PostMeta.tUseThisFile
			},
			multiple: bMulti
		} ).on( 'select', function(){
				...
				*/
				...
				if( bMulti == false ) {
					...
					if( target_preview.length > 0 ) {
						...
						var html = '<img src="' + attachment.sizes.thumbnail.url + '" /><a href="#" class="close media-modal-icon" title="' + ci_theme_PostMeta.tRemoveImage + '"></a>';
	...
	$( 'body' ).on( 'click', '.ci-upload-to-gallery', function( e ) {
		...
		var ciMediaGallery = wp.media( {
			frame: 'select',
			title: ci_theme_PostMeta.tSelectFiles,
			button: {
				text: ci_theme_PostMeta.tUseTheseFiles
			},
			multiple: true
			...
			ci_theme_featgal_AJAXUpdate( target_parent );
	...
	function ci_theme_featgal_UpdateIDs( preview_element ) {
		var ids = [];
	...
	function ci_theme_featgal_AJAXUpdate( gallery_container ) {
		...
		$.ajax( {
			type: 'post',
			url: ci_theme_PostMeta.ajaxurl,
			data: {
				action: 'ci_theme_featgal_AJAXPreview',
				ids: target_ids.val()
			},
			dataType: 'text',
			beforeSend: function() {
				target_preview.empty().html( '<p>' + ci_theme_PostMeta.tLoading + '</p>' );
			},
			success: function( response ) {
				if ( response == 'FAIL' ) {
					target_preview.empty().html( '<p>' + ci_theme_PostMeta.tPreviewUnavailable + '</p>' );
				} else {
					...
					$.each( response, function( key, value ) {
						$( '<div class="thumb"><img src="' + value.url + '" data-img-id="' + value.id + '"><a href="#" class="close media-modal-icon" title="' + ci_theme_PostMeta.tRemoveFromGallery + '"></a></div>' ).appendTo( target_preview );
					} );
				}
			}//success
		});//ajax
	...
	$( 'body' ).on( 'click', '.ci-media-manager-gallery .thumb a.close', function( event ) {
		...
		ci_theme_featgal_UpdateIDs( container );
	} );
	...
	if ( preview_areas.length > 0 ) {
		preview_areas.sortable( {
			update: function( event, ui ) {
				ci_theme_featgal_UpdateIDs( $( this ) );
			}
		} );
	}
